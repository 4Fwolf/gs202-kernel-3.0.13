...
#include <asm/arch/mt6575_i2c1.h>
 ...
 *   Returns: ERROR_CODE
 */
U32 i2c1_v1_set_speed (unsigned long clock, I2C_SPD_MODE mode, unsigned long khz)
{
    U32 ret_code = I2C_OK;
    ...
    printf("[i2c1_set_speed] Set sclk to %d khz (orig: %d khz)\n", sclk, khz);
    printf("[i2c1_set_speed] I2C Timing parameter sample_cnt_div(%d),  step_cnt_div(%d)\n", sample_cnt_div, step_cnt_div);
...
U32 i2c1_v1_init (void)
{
    ...
    /* Sset I2C speed mode */
    ret_code = i2c1_v1_set_speed(I2C_CLK_RATE, ST_MODE, MAX_ST_MODE_SPEED);
    if( ret_code !=  I2C_OK)
    {
        printf("[i2c1_init] i2c1_v1_set_speed error (%d)\n", ret_code);
        return ret_code;
    }
 ...
 *   Returns: ERROR_CODE
 */
U32 i2c1_v1_read(U8 chip, U8 *buffer, int len)
{
    U32 ret_code = I2C_OK;
    U8 *ptr = buffer;
    ...
    if (!len) {
        printf("[i2c1_read] I2C1 doesn't support len = 0.\n");
    ...
    while (1)
    {
        ...
        else if ( status & I2C_HS_NACKERR)
        {
            ...
            printf("[i2c1_read] transaction NACK error (%x)\n", status);
            break;
        }
        else if ( status & I2C_ACKERR)
        {
            ...
            printf("[i2c1_read] transaction ACK error (%x)\n", status);
            break;
        }
#if 1
        //else if (get_timer(tmo) > I2C_TIMEOUT_TH /* ms */) {
            ...
            printf("[i2c1_read] transaction timeout:%d\n", time_out_val);
            break;
        }
		time_out_val++;
#endif
...
U32 i2c1_v1_write (U8 chip, U8 *buffer, int len)
{
    ...
    /* CHECKME. mt65xx doesn't support len = 0. */
    if (!len)
    {   printf("[i2c1_read] I2C doesn't support len = 0.\n");
    ...
    while (1) {
        ...
        if ( status & I2C_TRANSAC_COMP) {
            ...
            printf("[i2c1_write] transaction NACK error\n");
            break;
        }
        else if ( status & I2C_ACKERR) {
            ret = 2;
            ret_code = I2C_WRITE_FAIL_ACKERR;
            printf("[i2c1_write] transaction ACK error\n");
            break;
        }
#if 1
        //else if (get_timer(tmo) > I2C_TIMEOUT_TH /* ms */ ) {
